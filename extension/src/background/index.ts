...
chrome.contextMenus.onClicked.addListener(async (info, tab) => {
  ...
  try {
    if (info.menuItemId === 'pm-open-vault') {
      chrome.tabs.create({ url: 'https://prompt-memory-prompt-memory-1.vercel.app/clips' });
      return;
    }
    ...
    try {
      if (request.action === 'SYNC_SESSION') {
        ...
        try {
          ...
          if (!user || !user.id) {
            sendResponse({ success: false, error: 'User session not synced. Please log in at https://prompt-memory-prompt-memory-1.vercel.app to authenticate your extension.' });
            return;
          }
